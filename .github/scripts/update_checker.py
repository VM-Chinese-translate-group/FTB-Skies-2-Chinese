...
def apply_exclusion_rules(file_set, exclusion_patterns, root_path):
    """
    Filters a set of file paths based on exclusion and inclusion (! negating) patterns.
    """
    ...
    for file_path in file_set:
        ...
        # If it was matched for exclusion, check for an exception
        if is_excluded:
            for pattern in inclusion_patterns:
                if relative_path.match(pattern):
                    is_excluded = False  # It's an exception, so we keep it
                    break
...
def main():
    ...
    exclusion_patterns = config.get('exclusionPatterns', [])  # Load exclusion patterns

    with open(info_file_path, 'r', encoding='utf-8') as f:
        local_version = json.load(f)['modpack']['version']
    print(f"Checking updates for: {pack_name} (ID: {pack_id})\nLocal version: {local_version}")
    inspect_output = run_command(['./CurseTheBeast', 'inspect', str(pack_id)])
    latest_version = next((line.split('│')[2].strip() for line in inspect_output.splitlines() if
                           'release' in line and line.count('│') > 2), None)
    if not latest_version: sys.exit("Error: Could not determine the latest version.")
    print(f"Latest version: {latest_version}")
    if local_version == latest_version:
        print("Already up to date. Exiting.")
        return
    print(f"New version found! Downloading {latest_version}...")
    temp_root = repo_root / 'temp_update'
    shutil.rmtree(temp_root, ignore_errors=True)
    extract_dir = temp_root / 'extracted'
    os.makedirs(extract_dir, exist_ok=True)
    run_command(['./CurseTheBeast', 'download', str(pack_id), '--output', str(temp_root / f"{pack_id}.zip")])
    print("Extracting package...")
    with zipfile.ZipFile(temp_root / f"{pack_id}.zip", 'r') as z:
        z.extractall(extract_dir)
    new_source_root = extract_dir / 'overrides'
    if not new_source_root.exists(): sys.exit("Error: 'overrides' directory not found in the extracted package.")
    ...
    for item in attention_list.get('filePatterns', []):
        ...
        print(f"  - Processing pattern: {pattern} (Ignore Deletions: {ignore_deletions})")
    ...
    for item in attention_list.get('folders', []):
        ...
        ignore_deletions = item.get('ignoreDeletions', False)
        print(f"  - Processing folder: {folder_rel_str} (Ignore Deletions: {ignore_deletions})")
    ...
    original_added_count = len(added_files)
    original_updated_count = len(updated_files)
    ...
    print(f"  - Filtered 'added' files: {original_added_count - len(added_files)} removed.")
    print(f"  - Filtered 'updated' files: {original_updated_count - len(updated_files)} removed.")

    if not any([updated_files, added_files, deleted_files]):
        print("Version updated, but no effective changes detected after filtering. Exiting.")
        return

    print("Applying changes...")
    for item in sorted(list(deleted_files), key=lambda p: len(p.parts), reverse=True):
        print(f"  - Deleting: {item.relative_to(source_dir) if item.is_relative_to(source_dir) else item}")
        if item.is_dir():
            shutil.rmtree(item)
        elif item.is_file():
            item.unlink()
    ...
    for item in all_to_copy:
        ...
        print(f"  - Copying: {item.relative_to(new_source_root)}")
    ...
    with open(info_file_path, "r+", encoding="utf-8") as f:
        data = json.load(f)
        data['modpack']['version'] = latest_version
        f.seek(0);
        json.dump(data, f, indent=2, ensure_ascii=False);
        f.truncate()
    print("Generating PR body...")
    pr_body = generate_pr_body(pack_name, latest_version,
    ...
    print("Setting outputs for GitHub Actions...")
