...
def main() -> None:
    ...
    if ftb_quests_lang_dir and ftb_quests_lang_dir.exists():
        ...
        # 新增 chapters 目录的定义
        # 注意：请确认这两个路径在你的项目中是真实存在的
        source_chapters_dir = 'Source/config/ftbquests/quests/chapters'
        output_chapters_dir = 'CNPack/config/ftbquests/quests/chapters' # 修改后的文件输出到CNPack中

        # 直接调用从 LangSpliter 导入的函数，并传入所有必需的参数
        # 检查一下源章节目录是否存在，如果不存在则不启用此功能
        ...
        if os.path.isdir(source_chapters_dir):
            ...
            # (根据LangSpliter.py的逻辑，传入空字符串是安全的)
